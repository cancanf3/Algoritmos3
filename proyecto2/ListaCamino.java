...
public class ListaCamino implements Grafo
{
	...
	try{		 
			  ...
			  Buffer = new BufferedReader(new FileReader(new File(args[1])));
			  String str = " ";
			  int ver = Integer.parseInt(Buffer.readLine());
			  int ari =  Integer.parseInt(Buffer.readLine());
			  ListaCamino digrafo = new ListaCamino();
			...
			for (int i=0 ; i < ari ; i++) {
				...
				digrafo.agregarArco(str1[0],Integer.parseInt(str1[3]),
									str1[1],str1[2]);
			}
	...
	private void dfs ( String ini ) {
		...
		for ( VerticeDir ver : this.lista ) {
			if ( ! ver.v.getId().equals(ini) ) {
				System.out.println( ver.v.getId() + " " + ver.visitado );
			}
		}
	}

	private void dfs_visitar ( VerticeDir ver ) {
		ver.visitado ++;

		for ( Arco arc : ver.l1 ) {
			String str = arc.getExtremoFinal().getId();
			for ( VerticeDir wer : this.lista ) {
				if ( wer.v.getId().equals(str) ) {
					dfs_visitar(wer);
				}
			}
		}
	...
	public ListaCamino(LinkedList<VerticeDir>listaady, int numArco) {						// Constructor opcional para Construir 
		this.numArco = numArco;															// un Grafo con una lista de adyasencias construida
        this.lista = listaady;
    }

	public ListaCamino(LinkedList<VerticeDir>listaady) {							// Constructor opcional para Construir 
    ...
    public ListaCamino clone() {
		...
		digrafoclon = new ListaCamino(listaclon, clonnumArco);	
		return digrafoclon;
	}
